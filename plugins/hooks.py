...
class PRAWHook(BaseHook):

    conn_name_attr = "reddit_conn_id"
    default_conn_name = "reddit_default"
    conn_type = "generic"
    hook_name = "praw"

    def __init__(self, reddit_conn_id: str = default_conn_name) -> None:
        super().__init__()
...
class RedisHookDecodeResponses(BaseHook):
    """
    ...
    def get_conn(self):
        ...
        if not self.redis:
            ...
            self.redis = Redis(host=self.host, port=self.port, password=self.password, db=self.db, decode_responses=True, **ssl_args)
